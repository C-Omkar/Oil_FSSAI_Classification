...
def check_oil(x, c_o):
    ...
    ctr = 0
    while(contain):
        if(oil_bounds[c_o][ctr][0]*x <= percentages[ctr] and oil_bounds[c_o][ctr][1]*x >= percentages[ctr]):
            ctr +=1
            if ctr == 21:
                return True
        else:
...
for c_oil in common_oils:
        checkx = np.linspace(0,1, 101)
        for x in checkx:
            if check_oil(x, c_oil):
                solutions.append([c_oil, round(100*x, 2)])

solutions = np.array(solutions)
print(solutions)
df = pd.DataFrame(solutions)
with pd.ExcelWriter("potential_oil1_compositionss.xlsx", mode="a", engine="openpyxl", if_sheet_exists="replace") as writer:
